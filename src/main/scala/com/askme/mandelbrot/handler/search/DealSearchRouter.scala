      ...
      requestInstance { (httpReq: HttpRequest) =>
        path("search" / "deal") {
          parameters('what.as[String] ? "", 'city ? "", 'area ? "", 'id ? "",
            ...
            'select ? "", 'screentype ? "", 'category ? "", 'featured ? "", 'dealsource ? "", "channelsort".as[Boolean]?false)
          { (kw, city, area, id, applicableTo, wantaggrs, size, offset, select,
             screentype, category, featured, dealsource, channelsort) =>
            val source = true
            val version = 1
            val fuzzyprefix = 2
            val fuzzysim = 1f
            ...
            respondWithMediaType(`application/json`) {
               ctx => context.actorOf(Props(classOf[DealSearchRequestCompleter], config, serverContext, ctx, DealSearchParams(
                  ...
                  filters = DealFilterParams(id, applicableTo, screentype, category, featured, dealsource, channelsort), page = PageParams(size, offset),
