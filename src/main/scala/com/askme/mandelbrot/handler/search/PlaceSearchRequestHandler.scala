      ...
      if (w.length > 0) {
        ...
        exactFirstFields.foreach {
          field: (String, Float) => {
            ...
            kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(10000000f)).must(nestIfNeeded(field._1, spanFirstQuery(nearQuery, termsExact.length + 1))).boost(field._2 * 131072f * 10000 * w.length * w.length * (searchFields.size + condFields.values.size + 1)))
          }
        }
        exactFields.foreach {
          field: (String, Float) => {
            ...
            val nearQuery = spanNearQuery.slop(0).inOrder(true)
            termsExact.foreach(nearQuery.clause)
            kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(10000000f)).must(nestIfNeeded(field._1, nearQuery)).boost(field._2 * 231072f * 10000 * w.length * w.length * (searchFields.size + condFields.values.size + 1)))
          }
        }
        ...
        kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(100000000000f)).must(strongMatch(searchFields, condFields, w, kw, fuzzyprefix, fuzzysim, esClient, index)))
        query = kwquery
