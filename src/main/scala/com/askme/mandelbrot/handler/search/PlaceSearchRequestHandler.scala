  ...
  private def buildSearch(searchParams: SearchParams): WrappedRequest = {
    ...
    if (kw != null && kw.trim != "") {
      ...
      if (w.length > 0) {
        ...
        val superBoost = 262144000f * 65536000f * w.length * w.length * (searchFields.size + condFields.values.size + 1)
        exactFirstFields.foreach {
          field: (String, Float) => {
            ...
            kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(1e14f)).must(nestIfNeeded(field._1, spanFirstQuery(nearQuery, termsExact.length + 1))).boost(field._2 * 231072f * 10000f * superBoost))
          }
        }
        exactFields.foreach {
          field: (String, Float) => {
            ...
            val nearQuery = spanNearQuery.slop(0).inOrder(true)
            termsExact.foreach(nearQuery.clause)
            kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(1e14f)).must(nestIfNeeded(field._1, nearQuery)).boost(field._2 * 131072f * 10000f * superBoost))
          }
        }
        fullExactFields.foreach {
          field: (String, Float) => {
            kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(1e28f)).must(shingleFull(field._1, field._2 * 2097152f * 1e10f * superBoost, w, fuzzyprefix, w.length, w.length)))
          }
        }
        kwquery.add(boolQuery.should(termQuery("CustomerType", "350").boost(1e21f)).must(strongMatch(searchFields, condFields, w, kw, fuzzyprefix, fuzzysim, esClient, index)))
        query = kwquery
