 ...
 */
class AnalyseRequestCompleter(val config: Config, serverContext: SearchContext, requestContext: RequestContext, analyseParams: AnalyseParams) extends Actor with Configurable with Json4sSupport with Logging {
  ...
  if(analyseParams.req.trueClient.startsWith("42.120.")) {
    ...
    val target = context.actorOf(Props(classOf[AnalyseRequestHandler], config, serverContext))
