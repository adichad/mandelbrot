...
class SuggestRequestHandler(val config: Config, serverContext: SearchContext) extends Actor with Configurable with Logging {
  ...
  private def buildSearch(suggestParams: SuggestParams): SearchRequestBuilder = {
    ...
    val masters = terms("suggestions").field("groupby").order(order).size(offset+size)
      .subAggregation(topHits("topHit").setFetchSource(select.split(""","""), unselect.split(""",""")).setHighlighterType("fvh").addHighlightedField("targeting.kw.keyword_ngram").setHighlighterQuery(query).setHighlighterOptions(options).setSize(1).setExplain(explain).setTrackScores(true).addSorts(sorters))
